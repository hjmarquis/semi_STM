require(glmnet)
W_hat_adaPCA_ridge = function(betak,betadelta,Skb, a = 1.1)
{
  p = nrow(betak)
  B = ncol(betak)
  Kp = ncol(Skb)
  ...
  for(j in 1:p)
  {
    tmp.fit = cv.glmnet(PSkb,betak[j,]-betadelta[j],alpha = 0, penalty.factor = ada.pen,
                        intercept = FALSE)
...
W_hat_adaPCA_lasso = function(betak,betadelta,Skb, a=1)
{
  p = nrow(betak)
  B = ncol(betak)
  Kp = ncol(Skb)
  ...
  for(j in 1:p)
  {
    tmp.fit = cv.glmnet(PSkb,betak[j,]-betadelta[j], penalty.factor = ada.pen)
    W.hat[j,]=drop(diag(1/Skb.sd) %*% eig.var$vectors %*% coef(tmp.fit,s="lambda.min")[-1])
    lambda[j] = tmp.fit$lambda.min
  }
