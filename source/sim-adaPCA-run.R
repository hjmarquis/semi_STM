...
nullout <- foreach(irep = 1:Nrep,.packages = c("MASS","glmnet")) %dopar%
  {
    ...
    betaSSL.adaPCA.ridge = betaSSL
    betaSSLk.adaPCA.ridge = betaSSLk
    thres.best.adaPCA.ridge = thres.best
    
 
    
    save(betadelta, betak, betadelta.thres, 
         W.hat.adaPCA.ridge, betaSSL.adaPCA.ridge,betaSSLk.adaPCA.ridge,thres.best.adaPCA.ridge, 
         file=file.name)
...
#------------------------------------------
  ...
  for(irep in 1:Nrep) 
  {
    ...
    betaG[,irep] = betaSSL.adaPCA.ridge
    betaGs[[irep]] = betaSSLk.adaPCA.ridge
    betaG.thres[,irep] = sign(betaG[,irep]) * pmax(0,abs(betaG[,irep]) - thres.best.adaPCA.ridge)
    betaGs.thres[[irep]] = sign(betaGs[[irep]]) * pmax(0,abs(betaGs[[irep]]) - thres.best.adaPCA.ridge)
