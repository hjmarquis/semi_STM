...
source("source/sim_gen_multiple.R")
...
nullout <- foreach(irep = 1:Nrep,.packages = c("MASS","glmnet")) %dopar%
  {
    ...
    rm(.Random.seed)
    rep.seed = sample(1)
    rep.seed = .Random.seed
    tmp = sim.gen.multiple(n,p,beta0,sigmaZ,mu,sigma,m)
    ...
    h1 = sdlp/(sum(dataS[[isetup]]$delta1))^0.3
    h2 = sdlp/(sum(dataS[[isetup]]$delta2))^0.3
    Sk = rep(0,p*2)
    Sk[1:p] = Sk_sym(lp,data$Z,
                     dataS[[isetup]]$X1,dataS[[isetup]]$delta1,
                     data$C,dnorm,h1)
    Sk[p+1:p] = Sk_sym(lp,data$Z,
                       dataS[[isetup]]$X2,dataS[[isetup]]$delta2,
                       data$C,dnorm,h2)
    ...
    for(ifold in 1:Nfold)
    {
      ...
      tmp.concord = (cv.concordance(lp.test, 
                                    dataS[[isetup]]$X1[i.test],dataS[[isetup]]$delta1[i.test],
                                    data$C[i.test]) + 
                       cv.concordance(lp.test, 
                                      dataS[[isetup]]$X2[i.test],dataS[[isetup]]$delta2[i.test],
                                      data$C[i.test]))
    ...
    for(ifold in 1:Nfold)
    {
      ...
      Sk.cv[1:p] = Sk_sym(lp.init.cv[i.train,ifold],data$Z[i.train,],
                          dataS[[isetup]]$X1[i.train],dataS[[isetup]]$delta1[i.train],
                          data$C[i.train],dnorm,h1)
      Sk.cv[p+1:p] = Sk_sym(lp.init.cv[i.train,ifold],data$Z[i.train,],
                            dataS[[isetup]]$X2[i.train],dataS[[isetup]]$delta2[i.train],
